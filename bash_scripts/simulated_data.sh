...
DATA_FILE="input/test_data_file.csv"
DATASET_NAME="simulated_data" # Enter the name of your dataset
...
KEGG_PATHWAYS=() # Enter KEGG pathway codes or leave blank to find all pathways with overlapping genes. Separate like: ("04670" "05171")
CUSTOM_PATHWAYS=(test_network.graphml) #("modified_network.graphml") #Put custom networks in the input folder
...
run_rule_determination() {
    ...
    if [ ${#KEGG_PATHWAYS[@]} -gt 0 ]; then
        ...
        /usr/bin/time -v \
        python3 code/pipeline_class.py \
            --data_file "$DATA_FILE" \
...
run_importance_score() {
    ...
    if [ ${#KEGG_PATHWAYS[@]} -gt 0 ]; then
        ...
        /usr/bin/time -v \
        python3 code/importance_scores.py \
            --dataset_name "$DATASET_NAME" \
            --list_of_kegg_pathways $KEGG_PATHWAYS_ARGS
    else
...
run_relative_abundance() {
    ...
    for (( i=0; i<${#CONTROL_GROUPS[@]}; i++ )); do
        ...
        /usr/bin/time -v \
        python3 code/relative_abundance.py \
            --dataset_name "$DATASET_NAME" \
...
run_attractor_analysis() {
    ...
    /usr/bin/time -v \
    python3 code/attractor_analysis.py \
        --dataset_name "$DATASET_NAME" \
