...
class Object(pygame.sprite.Sprite):
    ...
    def move(self, events, rect, game):
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN and rect.collidepoint(mouse_pos) and game.node_being_moved is None:
                self.moving = True
    ...
    def line_with_arrow(self):
        mouse_pos = pygame.mouse.get_pos()
        direction = (mouse_pos[0] - self.position[0], mouse_pos[1] - self.position[1])
        distance = math.sqrt(direction[0] ** 2 + direction[1] ** 2) + 1e-5
        unit_direction = (direction[0] / distance, direction[1] / distance)
    ...
    def connect_to_object(self, events, objects):
        ...
        if self.is_drawing_line and keys[pygame.K_1]:
            for obj in objects:
    ...
    def disconnect_objects(self, objects):
        ...
        if self.is_drawing_line and keys[pygame.K_t]:
            for obj in objects:
                if obj.rect.collidepoint(mouse_pos):
    ...
    def remove_all_connections(self, objects, just_killed):
        ...
        if keys[pygame.K_y] or just_killed:
            for obj in objects:
                if obj.rect.collidepoint(mouse_pos):
                    self.remove_connection(obj)
    ...
    def run_simulation(self, draw_object_function):
        ...
        if keys[pygame.K_TAB] and self.can_update:
    ...
    def lock_state(self, rect):
        ...
        if keys[pygame.K_3] and rect.collidepoint(mouse_pos) and self.can_update:
            self.update_time = pygame.time.get_ticks()
