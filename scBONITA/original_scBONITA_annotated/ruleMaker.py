...
class ruleMaker:
    def __makeToolBox(self, graph):
        ...
        # Specifies the weights of the errors (should all be -1)
        weightTup = (-1.0,)
        for i in range(len(self.nodeList) - 1):
            weightTup += (-1.0,)

        # MAKE TYPES
        # Make a fitness minimization function as the objective
        creator.create("FitnessMin", base.Fitness, weights=weightTup)

        # Create a class of individuals that are lists with a fitness specified by the FitnessMin function
        ...
        # Allows for random bitstrings to be created (used to randomly select possible rules for nodes)
        toolbox.register("genRandomBitString", self.__genBits)

        # Register individuals with the ability to generate random bitstrings
        toolbox.register(
            "individual",
            tools.initIterate,
            creator.individual,
    ...
    def __init__(
        ...
        # Creates a dictionary of the nodes (keys = node names, values = node indices in the list)
        for i in range(0, len(nodeList)):
            nodeDict[
                nodeList[i]
            ] = i  # constructs the node dict so we can easily look up nodes

        # This counter is used to index where each node's possible rules are in the individual
        counter = int(0)

        # Iterate through the length of the list of nodes and store each possible rule combiantion for each node
        for i in range(0, len(nodeList)):

            # Create a list of the incoming connections to this node
            predecessors_temp = list(
                graph.predecessors(nodeList[i])
            )

            # Create a list of the outgoing connections for this node
            successors_temp = list(
                graph.successors(nodeList[i])
            )
            
            # Keep track of how many outgoing connections this node has
            succnum.append(len(successors_temp))

            # Get the index of each possible incoming node
            possibilitytemp = [nodeDict[predder] for predder in predecessors_temp]

            # Add the list of possibile incoming node names to a larger list
            possibilityLister.append(list(possibilitytemp))

            # Find correlation between the predecessors and this node
            nodeData = (
                self.binMat[self.nodePositions[i], :].todense().tolist()[0]
            )

            # Store the correlations between this node and the predecessors
            predCorr_temp = (
                []
            )

            # For each of the incoming nodes:
            for k in predecessors_temp:

                # Find the index in the dataset of the predecessor from the gene list
                predIndex = self.geneList.index(k)

                # Find the binarizedexpression data for this incoming node
                predData = (self.binMat[predIndex, :].todense().tolist()[0])

                # Calculate the correlation between this node and the predecessor
                mi, pvalue = spearmanr(nodeData, predData)
                mi = abs(mi)

                # Store the correlation between this incoming node and the current node
                if np.isnan(mi):
                    predCorr_temp.append(0)
                else:
                    predCorr_temp.append(mi)  # store the calculated correlation

            # Sort and store a list of the top 3 predecessors based on their correlation to this node
            predecessors_final = sorted(
                zip(predecessors_temp, predCorr_temp),
                reverse=True,
                key=lambda corrs: corrs[1])[:3]
            
            # Store the r-values from the correlations for the top 3 nodes
            self.rvalues.append(
                sorted(predCorr_temp, reverse=True)[:3]
            )

            # Store the names of the top 3 incoming nodes to a permanent list
            self.permList.append([pred[0] for pred in predecessors_final])

            # For each of the final predecessors, write their connection to the current node in the new network
            ...
            # Create a zipped list of the incoming node indices and "empty" (corr_tuple[0] = name of the predecessor node)
            # (A, B, C), ("empty", "empty", "empty")
            withNones = zip(
                [nodeList.index(corr_tuple[0]) for corr_tuple in predecessors_final], # List of incoming node indices
                itertool.repeat("empty"),
            )

            # Create a list of the possible combinations of the incoming nodes
            # (A, B, C), (A, B, "empty"), (A, "empty", "empty"), (A, "empty", C), ...
            possibilities = list(itertool.product(*withNones))

            # Find each possible combination of rules based on the incoming nodes
            for j in range(0, len(possibilities)):

                # Change the current possibility from a tuple to a list
                # (A, B, "empty") -> [A, B, "empty"]
                possibilities[j] = list(possibilities[j])

                # Remove any values of "empty", this allows for two- and one-incoming node possibilities
                # [A, B]
            ...
            # If there are any empty lists at the end, remove them (i.e. ["empty", "empty", "empty"])
            ...
            # This goes through the NetworkX graph and extracts whether a node is activating "a" or inhibiting "i"
            ...
            # Append the possibile combinations to a list of possible AND connections
            andNodeList.append(possibilities)

            # Append the activities to a list (0 if activating, 1 if inhibiting)
            andNodeInvertList.append(activities)

            # Keep track of the length of the possible combinations being added (so we know where we are in the individual)
            andLenList.append(len(possibilities))

            # Create a list of the last node's activity
            ...
            # This keeps track of where the start of the rule combination for this node starts and ends
            # If this node has 7 possibilities and is the first node, this node will start at 0 
            # in the individualand the next node will start at index 7
            individualParse.append(counter)
            counter = counter + len(possibilities)

        # The size of the individual is the total number of possibilities for all nodes at the end
        self.size = counter

        # We append the last counter to the list, index of the end of the final node
        individualParse.append(counter)

        # Size of the individual
        self.individualParse = individualParse  # index of start value of current node on the individual

        # List of each possible rule combination
        self.andNodeList = andNodeList  # shadow and node inputs

        # List of whether or the incoming nodes in the rules are activating or inhibiting
        self.andNodeInvertList = andNodeInvertList

        # Number of possible rule combinations for each node
        self.andLenList = andLenList

        # List of the incoming node names
        self.possibilityList = possibilityLister

        # List of whether the last node is activating or inhibitory (probably intended to store every node's activity)
        self.possibilityInverter = possibilityInverter

        # The number of nodes in the network
        self.nodeNum = len(nodeList)

        # Sets up the genetic algorithm parameters
        self.params = self.Params()
        self.params._Params__simParams()

        # Creates the genetic algorithm toolbox
        self.__makeToolBox(graph)

        # NetworkX graph object that we are building
        self.ruleGraph = ruleGraph

        # Dictionary of the nodes (key = name, value = index in node list)
        self.nodeDict = nodeDict

        # Number of outgoing connections for each node
        self.successorNums = succnum

        # Print out some info about the individuals
    ...
    def __update_upstream(self, node, newUpstreams):
        """
        Updates the node based on new upstream nodes, goes through the process of finding
        all possible rule combinations and if the nodes are activating or inhibiting
        """
        # Iterates through each of the new possible rules and finds all possible rules
        ...
        # create a list of the activities of each upstream node
        activities = []  # list to store activities of nodes (a vs i)
        for sequence in possibilities:
            activity = []
        ...
        # Update the list of node possibilities and inversion rules for the individual
    ...
    def __genRandBits(self):
        """
        Generates a random bitstring corresponding to the number of possible 
        rule combinations for each node in the individual
        (Used to select random rules)
        """
    ...
    def __findEnd(self, node):
        """
        Finds the index in the individual where the rule combinations for this node end
        (either the end of this node if its the last one, or the start of the next node
        in the individualParse list containing the start indices for each node's combinations)
        """
    ...
    def __findPopBest(self, population):
        ...
        # Set up initial dummy values for the variables
        saveVal = -1
        minny = float("Inf")
        ...
        for i in range(len(population)):
            ...
            # If the sum of the individual's fitness values is less than the minimum
            if np.sum(population[i].fitness.values) < minny:

                # Sum the individuals fitness value and save that as the minimum error
                minny = np.sum(population[i].fitness.values)

                # Save the index of the lowest error individual
                saveVal = i
        
        # The lowest error individual's index in the population
        ultimate = population[saveVal]

        # The lowest error individual's fitness value
    ...
    def __NP(self, individual, model, cells, params, KOs, KIs, scSyncBoolC):
        ...
        Uses C code to simulate a synchronous update method for the network
        """
        cellArray = []

        # set up knockin and knockout lists, a list of 0's and 1's the size of the node list
    ...
    def __genBits(self):
        ...
        # Generate random 1's and 0's for the individual
        ...
        for node in range(0, len(self.nodeList)):
            ...
            # Find the start of the current node
            start = self.individualParse[node]

            # If the length of the node combinations is greater than 1:
            if (end - start) > 1:

                # Create a random string of bits and increase the counter by 1
                counter = 0

                # While there are more than 5 combinations selected for this node and the counter is less than infinity
                while np.sum(startInd[start:end]) > 5 and counter < float("Inf"):

                    # Choose a random value between the start and end of the node
                    chosen = math.floor(random() * (end - start))

                    # Set the value of that node to 0
            ...
            # If there is only one possible rule, set it to 1
            elif (end - start) == 1:
                startInd[start] = 1

        # Return a deep copy of the ruleMaker object and the bitstring
    ...
    def __sortNondominatedAdapt(self, individuals, k, first_front_only=False):
        ...
        # Create a dictionary with (key = fitness value, value = list of individuals with that fitness value)
        map_fit_ind = defaultdict(list)
        for ind in individuals:
            map_fit_ind[ind.fitness].append(ind)

        # Create a list of the fitness values
        ...
        for i, fit_i in enumerate(fits):
            ...
            for fit_j in fits[i + 1 :]:
                ...
                # If the first fitness is greater
                if self.__dominated(fit_i, fit_j):

                    # Add that fit_j was dominated by an opponent
                    dominating_fits[fit_j] += 1

                    # Append fit_j to the list of fitnesses that fit_i won against
                    dominated_fits[fit_i].append(fit_j)
                
                # If the second fitness is greater
                elif self.__dominated(fit_j, fit_i):

                    # Add that fit_i was dominated by an opponent
                    dominating_fits[fit_i] += 1

                    # Add fit_i to the list of fitnesses that fit_j won against
                    dominated_fits[fit_j].append(fit_i)

            # If the current fitness has not been dominated, append it to the top of the list of current winners
            if dominating_fits[fit_i] == 0:
                current_front.append(fit_i)
        

        fronts = [[]]
        # For each fitness in the current front, add the individuals with that fitness to the list of fronts
        for fit in current_front:
            fronts[-1].extend(map_fit_ind[fit])

        # Get the length of the best individuals
    ...
    def __dominated(self, ind1, ind2):
        ...
        # Finds if individual1's wvalues are greater than individual2's wvalues
        mean1 = np.mean(ind1.wvalues)
        mean2 = np.mean(ind2.wvalues)
        std1 = np.std(ind1.wvalues)

        # If the value for ind1 is greater than ind2, return True
        if mean1 > mean2:
            return True
        elif mean1 < mean2:
            return False
    ...
    def __selNSGA2(self, individuals, k):
        ...
        # Sort all of the individuals based on their fitness
        pareto_fronts = self.__sortNondominatedAdapt(individuals, k)

        # Assign a crowding distance to each of the individuals
        for front in pareto_fronts:
            self.__assignCrowdingDist(front)

        # Return a list of chosen pareto fronts with the smallest crowding distance of length k
    ...
    def __bitList(self, n, x):
        """
        Turns a string of bits into a list of bit integers
        """
        # Creates a list of 1 and 0 integers based on the binary representation of n
        templist = [1 if digit == "1" else 0 for digit in bin(n)[::-1]]

        # Adds 0's to the templist if there are fewer values than x
        while len(templist) < x:
            templist.append(0)

        # Removes items from the templist if there are more than x
        while (len(templist)) > x:
            templist.pop()
    ...
    def writeModel(self, individual, model):
        """
        Iterate over the nodes in the individual to generate a BooleanNet representation for all of the 
        nodes in the model
    ...
    def __findInEdges(self, model, node):
        ...
        """
        inEdges = []
        # For each 'and' node in the model
        for lister in model.andNodeList[node]:
            # Convert the list of 'and' rules into a tuple
            tempTup = tuple(lister)

            # Convert the rule combinations into a set and append it to a list of incoming edges
        ...
        for i in range(len(rule)):
            ...
            if rule[i] == 1:
                ...
                for j in range(len(rule)):
                    ...
                    # If that node is also 1 and they are not the same nodes
                    if rule[j] == 1 and not i == j:

                        # if the incoming edges for node i are a subset of the incoming edges for node j
                        if inEdges[i].issubset(inEdges[j]):
                            
                            # Set node j in the rule to 0 (node i is sufficient and a subset of node j)
                            rule[j] = 0
        return rule
    ...
    def __writeNode(self, currentNode, nodeIndividual, model):
        ...
        # Find the list of and rules for the current node
        ...
        # Find the list of inversion rules for the current node
        ...
        writenode = (f"{model.nodeList[currentNode]} *= ")

        # Find the incoming edges for the current node
        inEdges = self.__findInEdges(model, currentNode)

        # Simplify the rule for the current node (nodeIndividual = rule when being called in singleCell.py)
        nodeIndividual = self.__simplifyRule(nodeIndividual, inEdges)

        # If the list of and nodes for the current node is 0 or the sum of the rule is 0
        if model.andLenList[currentNode] == 0 or sum(nodeIndividual) == 0:

            # If there are no inputs, write out that the node signals to itself
            return f'{writenode} {model.nodeList[currentNode]}'  
        
        # If there is one input to the rule
        elif len(andNodes) == 1:

            value = ""

            # If the node is activating (0 in andNodeInvertList), write out the node's name
            if andNodeInvertList[0][0] == 0:
                value = value + model.nodeList[andNodes[0][0]]

            # If the node is inhibiting (1 in andNodeInvertList), write out "not {node's name}"
            else:
        ...
        # If there is more than one input to the node
        else:
            ...
            for andindex in range(len(nodeIndividual)):
                ...
                if nodeIndividual[andindex] == 1:
                    ...
                    # For each node in the and rule
                    for addnode in range(1, len(andNodes[andindex])):
                        # Add " and " to the string
                        newval = newval + " and "

                        # Add " not " to the string it the node is inverted
                        if andNodeInvertList[andindex][addnode]:
                            newval = newval + " not "

                        # Add the node in the rule to the string
                        newval = newval + self.nodeList[andNodes[andindex][addnode]]
                    
                    # Close the and rule with closing parentheses
                    orset.append(newval + ")")
    ...
    # Deap genetic algorithm simulation
