...
class singleCell(ruleMaker):
    ...
    def __init__(
        ...
        # Reads in the data file and extracts the cells
        self.sampleList = open(dataName).readline().rstrip().split(sep)[1 : len(self.sampleList)]
        
        # If there are more than 15000 cells, sample them randomly
        if len(self.sampleList) >= 15000 or sampleCells:
            ...
            # Load the expression data from the datafile, use the sampled cell indices
            ...
            # Create the list of genes, the list of samples, and the expression matrix
        ...
        # If there are fewer than 15000 cells or sample indices is false, use the whole dataset
        else:
            data = np.loadtxt(
                dataName,
                delimiter=sep,
        ...
        # Get rid of any 0 cvalues
        self.expMat.eliminate_zeros()

        # Binarize the expression matrix based on the threshold (> threshold = 1, < threshold = 0)
        self.binMat = preprocessing.binarize(
            self.expMat, threshold=binarizeThreshold, copy=False
        )
        
        # Set the max number of nodes and samples
