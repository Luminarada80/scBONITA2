...
def generate_dataset_no_check(chunks, ruleset, num_cells, num_genes):
    matrix_chunks = []
    total_mismatches = 0
...
def generate_dataset_with_check(chunks, ruleset, num_cells, num_genes):
    matrix_chunks = []
    num_tries = 0
    num_successes = 0
    ...
    return np.hstack(matrix_chunks), num_successes

def simulate_network_no_check(num_genes, num_cells, chunks):
    ...
    matrix, total_mismatches = generate_dataset_no_check(chunks, ruleset, num_cells, num_genes)
    ...
    percent_error_file_path = f'{sim_data_file_paths["sim_ruleset"]}/percent_error_log.txt'
...
def simulate_network_with_check(num_genes, num_cells, chunks):
    ...
    with alive_bar(0, bar='classic2', spinner='dots_waves') as bar:
        while True:
            ...
            rule_dict, network = generate_network(num_genes, test_network)
            ruleset = [rule.replace('Gene', '') for _, rule in rule_dict.items()]
            matrix, num_successes = generate_dataset_with_check(chunks, ruleset, num_cells, num_genes)

            if isinstance(matrix, np.ndarray):
                # print(f'\t\tAttempt {attempt_num} successful')
                break
            # elif num_successes > 0:
                # print(f'Attempt {attempt_num} failed after {num_successes}')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Specify if you want to make sure that the simulated data has no errors compared to the ruleset. True = allows errors to pass'
    )

    results = parser.parse_args()

    num_genes = getattr(results, 'num_genes')
    num_cells = getattr(results, 'num_cells')
    num_chunks = getattr(results, 'num_chunks')
    allow_mismatches = getattr(results, 'allow_mismatches')

    if allow_mismatches == "True":
        print('Simulating the dataset, mismatches allowed')
        simulate_network_no_check(num_genes, num_cells, num_chunks)

    else:
        print('Simulating the dataset, no mismatches allowed')
        simulate_network_with_check(num_genes, num_cells, num_chunks)

    end_time = time.time()
    elapsed_time = end_time - start_time
    formatted_time = str(timedelta(seconds=round(elapsed_time)))

    print(f'Generated simulated data with {num_genes} genes and {num_cells*num_chunks} cells in {formatted_time}')